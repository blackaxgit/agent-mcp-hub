...
- **Model namespaces do NOT overlap between agents — a single `model` cannot serve a fan-out.** `o3` is meaningless to `agy`, `gemini-3.6-flash-low` is meaningless to `codex`, and `opencode` namespaces its own (`opencode/big-pickle`). That is why `run_all` takes a per-agent `models` map and `review_change` takes `runnerModel`/`reviewerModel`; the scalar `model` is only a fallback, and is genuinely useful only when every agent in the call is the same CLI. Resolution is `models[agent] ?? model ?? <CLI default>`. **Every value in the map goes through the same `MODEL_RE` allowlist** — the map must never become a hole in the argument-injection defence — and unknown agent KEYS are rejected with the enabled list rather than silently ignored — with **one documented exception: `__proto__`**, which `z.record` loses before the check can see it (it rebuilds by assignment, hitting the inherited setter). That key is ignored rather than reported; it can never name an agent so it cannot select a model, and a test pins that no value from it reaches argv. **Do not "fix" this with `z.custom`** — a hand-rolled schema is unrepresentable in JSON Schema (`Custom types cannot be represented in JSON Schema`) and breaks `tools/list` for every MCP client, which is far worse than a missing error message. All of it (scalar and per-agent) is surfaced in the `MCP_CONFIRM` gate, because anything that reaches a CLI's argv must be visible to the human approving the run.

  Discovery is uneven and partly untrustworthy: `opencode models` (~400 ids), `cursor-agent models` (~195 lines of prose) and `agy models` (11 ids) list them; `codex` has no such command (it reads `~/.codex/config.toml`) and `claude` has none either — **`claude models` is not a subcommand, it is parsed as a PROMPT**, so its tidy "model lineup" output is model-generated text and must never be treated as a source of truth. Asking an agent "which model are you?" is likewise unreliable — codex answers `gpt-5.6-sol` when given `gpt-5.6-terra`. To verify a model flag is honored, pass a bogus id and check that the CLI *rejects* it.
